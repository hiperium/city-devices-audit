...
CREATE_FUNCTION_PATH="/var/tmp/trails-create/create-function.jar"
CREATE_DEPENDENCIES_PATH="/var/tmp/trails-create/create-function-libs.zip"
while [ ! -f "$CREATE_FUNCTION_PATH" ] || [ ! -f "$CREATE_DEPENDENCIES_PATH" ]; do
    sleep 3
done
echo "DONE!"
...
echo ""
echo "CREATING TRAIL CREATE LAMBDA LAYER..."
awslocal lambda publish-layer-version               \
    --layer-name 'trail-create-function-layer'      \
    --description "Trail Create dependencies"       \
    --zip-file fileb://$CREATE_DEPENDENCIES_PATH    \
...
awslocal lambda create-function                                                                      \
    ...
    --layers 'arn:aws:lambda:us-east-1:000000000000:layer:trail-create-function-layer:1'             \
...
echo ""
echo "ADDING TRAIL CREATE FUNCTION AS EVENTBRIDGE TARGET..."
